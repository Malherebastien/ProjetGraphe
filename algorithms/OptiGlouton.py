...
class OptiGlouton:
    ...
    def optimise_glou(self):
        ...
        for i in range(0, len(self.sortie_glou)):
            self.sortie_opti.append(self.sortie_glou[i])

        somme = 0
        changement = 1
        while changement != 0:
            changement = 0
            for i in range(0, len(self.sortie_opti)-1):
                for j in range(0, len(self.sortie_opti)-1):
                    if abs(i - j) > 1:
                        if self.est_croise(i, j):
                            print("application", i, " et ", j)
                            self.sortie_opti = self.echange_sommet(i, j)
                            changement += 1
        for i in range(0, self.nb_point-1):
            somme += np.math.sqrt((self.points[self.sortie_opti[i]].x - self.points[self.sortie_opti[i+1]].x)**2 +
                                  (self.points[self.sortie_opti[i]].y - self.points[self.sortie_opti[i+1]].y)**2)
        print(somme)
        return self.sortie_opti
    ...
    def est_croise(self, i, j):
        ...
        """
        abx = self.points[self.sortie_opti[i+1]].x - self.points[self.sortie_opti[i]].x
        aby = self.points[self.sortie_opti[i+1]].y - self.points[self.sortie_opti[i]].y
        acx = self.points[self.sortie_opti[j]].x - self.points[self.sortie_opti[i]].x
        acy = self.points[self.sortie_opti[j]].y - self.points[self.sortie_opti[i]].y
        adx = self.points[self.sortie_opti[j + 1]].x - self.points[self.sortie_opti[i]].x
        ady = self.points[self.sortie_opti[j + 1]].y - self.points[self.sortie_opti[i]].y

        det1 = abx * acy - aby * acx
        det2 = abx * ady - aby * adx

        cdx = self.points[self.sortie_opti[j + 1]].x - self.points[self.sortie_opti[j]].x
        cdy = self.points[self.sortie_opti[j + 1]].y - self.points[self.sortie_opti[j]].y
        cax = -acx
        cay = -acy
        cbx = self.points[self.sortie_opti[i + 1]].x - self.points[self.sortie_opti[j]].x
        cby = self.points[self.sortie_opti[i + 1]].y - self.points[self.sortie_opti[j]].y
    ...
    def echange_sommet(self, index1, index2):
        ...
        """
        tab = []
        for i in range(0, len(self.sortie_opti)):
            tab.append(self.sortie_opti[i])
        if index1 < index2:
            i = index1+1
            k = index2
        else:
